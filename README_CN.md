...
│   │   │   │   │   └── GroupDao.kt          # 分组数据访问对象
│   │   │   │   ├── model/
│   │   │   │   │   ├── Script.kt             # 脚本和动作的数据模型
│   │   │   │   │   └── ScriptGroup.kt        # 脚本分组数据模型
│   │   │   │   └── repository/
│   │   │   │       ├── ScriptRepository.kt   # 脚本仓储
│   │   │   │       └── GroupRepository.kt    # 分组仓储
│   │   │   ├── di/
...
│   │   │   │   ├── ImageMatchingService.kt           # OpenCV 图像匹配
│   │   │   │   └── TextRecognitionService.kt         # ML Kit 文字识别
│   │   │   └── ui/
│   │   │       ├── AutoBotApp.kt             # 主应用导航
│   │   │       ├── theme/                    # Material 3 主题
│   │   │       │   ├── Color.kt
...
- **查找图片**：使用 OpenCV 的模板匹配（支持调试、阈值和结果变量）
- **查找文字**：使用 ML Kit 的 OCR 文字识别（支持中文和英文、调试、阈值和结果变量）

### 3. 流程控制

- **循环**：完整实现的重复执行动作（N次或无限）
- **条件**：根据图像/文字进行分支（支持真/假分支）
- **注释**：为脚本添加备注
...
- 通过拖拽重新排序动作
- 使用 Room 数据库保存/加载脚本
- **脚本分组**：将脚本组织成分组以便更好地管理

### 5. 变量系统

- 所有动作的完整变量支持（点击、滑动、长按、文本输入、延时、按键、截图、注释）
- 变量引用格式：`${变量名}` 或 `${变量名.属性}`
- 查找图片和查找文字结果可保存到变量
- 用于自定义变量赋值的设置变量动作
...
- [x] **循环逻辑实现** - 在 `AutomationEngine.executeActions()` 中实现实际的循环执行，包括循环开始/结束检测和循环计数器管理
- [x] **条件分支实现** - 添加实际的条件评估和真/假分支执行
- [x] **条件序列化** - 修复 `ScriptConverters` 以正确序列化/反序列化 `Condition` 动作中的 `trueBranch` 和 `falseBranch`
- [x] **截图功能** - 在 `takeScreenshot()` 中实现实际的屏幕捕获和文件保存
- [x] **图像识别（查找图片）** - 使用 OpenCV 实现模板匹配，支持调试模式和结果变量
- [x] **文本输入修复** - 使用剪贴板方法实现实际的文本输入
- [ ] **颜色匹配** - 实现 `COLOR_MATCH`/`COLOR_NOT_MATCH` 条件的颜色检测和比较逻辑

### 中优先级 - 附加功能

- [ ] **脚本导入/导出** - 添加备份和恢复脚本的功能（推荐 JSON 格式）
- [x] **脚本分组** - 添加分组/分类功能用于组织脚本
- [ ] **双击动作** - 向 `ScriptAction` 添加 `DoubleTap` 动作类型并在引擎中实现
- [x] **脚本执行暂停/恢复** - 验证并测试暂停/恢复功能
- [x] **坐标记录器** - 测试并确保从屏幕记录坐标功能正常工作
- [x] **变量系统（完整）** - 添加将 FindImage/FindText 结果存储在变量中的支持，并在所有动作中使用变量

### 低优先级 - 增强功能

- [x] **完整变量系统** - 为所有动作添加完整的变量存储和使用支持
- [x] **OCR 文字识别** - 添加使用 ML Kit 识别屏幕上文字的功能
- [ ] **云共享** - 添加脚本共享功能
