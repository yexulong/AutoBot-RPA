...
- **查找图片**：模板匹配（基础实现）
- **查找文字**：使用 ML Kit 的 OCR 文字识别（支持中文和英文）
...
- [ ] **循环逻辑实现** - 在 `AutomationEngine.executeActions()` 中实现实际的循环执行，包括循环开始/结束检测和循环计数器管理
...
- [x] **变量系统（部分）** - 添加将 FindImage 结果存储在变量中的支持（部分实现已存在）

### 低优先级 - 增强功能

- [ ] **完整变量系统** - 为所有动作添加完整的变量存储和使用支持
